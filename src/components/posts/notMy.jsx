...
function NotMy(props) {
    const id = props.location.state.id
    const { socket, notify } = props;
    const history = useHistory();
    const [posts, setPosts] = useState(false)
    socket.on("notMyPostUpdate", (updatedPosts) => {
        setPosts({ posts: updatedPosts })
    })
    useEffect(() => {
        axios.get("https://sonet34.herokuapp.com/api/post/notMy", { params: { userId: id } })
...
export default withRouter(NotMy);